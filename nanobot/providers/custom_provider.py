...
from nanobot.models_config import AVAILABLE_MODELS, get_model_config
from nanobot.providers.base import LLMProvider, LLMResponse, ToolCallRequest


class CustomProvider(LLMProvider):
    """Provider that supports both OpenAI-compatible and Anthropic-compatible APIs.

    Detects model type from AVAILABLE_MODELS and uses the appropriate endpoint,
    auth header, and request/response format.
    """
    ...
    async def chat(
        ...
        Automatically detects model type and uses appropriate API format:
        - OpenAI-compatible: POST to /chat/completions with Authorization: Bearer
        - Anthropic-compatible: POST to /messages with x-api-key header
        """
        model_id = model or self.default_model
        model_config = get_model_config(model_id) or {}
    ...
    async def _chat_anthropic(
        ...
        """Send request using Anthropic-compatible API format."""
    ...
    def _parse_anthropic_response(self, data: dict[str, Any]) -> LLMResponse:
        """Parse Anthropic-compatible API response."""
        ...
        elif stop_reason in ("max_tokens", "end_turn"):
            finish_reason = "stop"
        ...
        return LLMResponse(
            ...
            thinking_blocks=content_blocks if any(b.get("type") == "thinking" for b in content_blocks) else None,
        )
    ...
    def _prepare_messages_anthropic(
        ...
        """Prepare messages for Anthropic-compatible API.

        Returns: (system_prompt, chat_messages)
        - system_prompt: The system message content
        - chat_messages: List of user/assistant messages in Anthropic format
        """
        system_prompt = ""
        ...
        for msg in messages:
            ...
            if role == "user":
                chat_messages.append({
                    "role": "user",
                    "content": self._convert_content_anthropic(content),
                })
            elif role == "assistant":
                    ...
                    for tc in msg["tool_calls"]:
                        if isinstance(tc, dict):
                            assistant_content.append({
                                ...
                                "name": tc.get("function", {}).get("name", ""),
                                "input": self._parse_args(tc.get("function", {}).get("arguments", "{}")),
                            })
                # Remove empty text content if there are tool uses
                assistant_content = [c for c in assistant_content if not (c.get("type") == "text" and not c.get("text"))]
                ...
                chat_messages.append({
                    ...
                    "content": [{
                        "type": "tool_result",
                        "tool_use_id": self._normalize_tool_call_id(msg.get("tool_call_id", "toolu_0")),
