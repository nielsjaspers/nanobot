...
def test_config_defaults_to_opencode_go_kimi() -> None:
    config = Config()

    assert config.agents.defaults.provider == "custom"
    assert config.agents.defaults.model == "kimi-k2.5"
    assert config.providers.custom.api_base == "https://opencode.ai/zen/go/v1"
